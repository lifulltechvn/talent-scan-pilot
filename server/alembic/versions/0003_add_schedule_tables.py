...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column("slot_id", postgresql.UUID(as_uuid=True), sa.ForeignKey("schedule_slots.id"), nullable=True),
        sa.Column("token", sa.String(255), unique=True, nullable=False, index=True),
        sa.Column("status", sa.String(20), server_default="'pending'"),  # pending / booked / cancelled

...
export function LoginPage() {
  ...
  const validateEmail = (v: string) => !v ? t('emailRequired') : !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(v) ? t('emailInvalid') : null;
  const validatePassword = (v: string) => !v ? t('passwordRequired') : v.length < 6 ? t('passwordMinLength') : null;

...
export function InterviewCoachingModal({ interviewId, onClose }: { interviewId: string; onClose: () => void }) {
  ...
  const run = async () => {
    ...
    try {
      ...
      setResult(data);
    } catch (e: any) { setError(e.response?.data?.detail || 'Không thể phân tích'); }
    setLoading(false);
  };
      ...
      <div className="bg-white rounded-2xl shadow-xl w-full max-w-md m-4 max-h-[80vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
        <div className="flex items-center justify-between px-5 py-4 bg-accent rounded-t-2xl">
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-[15px] font-semibold text-white">AI Interview Coaching</h2>
          </div>
        ...
        <div className="p-5">
          {!result && !loading && !error && (
            <div className="text-center py-4">
              <p className="text-[12px] text-text-muted mb-3">AI phân tích chất lượng feedback của interviewer & phát hiện bias</p>
          ...
          {loading && (
            <div className="text-center py-6">
              ...
              <p className="text-[13px] text-text-muted">Đang phân tích...</p>
            </div>
          )}
            ...
            <div className="space-y-3">
              ...
              <div className={`p-3 rounded-lg text-center ${result.overall_quality === 'good' ? 'bg-emerald-50' : result.overall_quality === 'needs_improvement' ? 'bg-amber-50' : 'bg-red-50'}`}>
                <span className={`text-[14px] font-bold ${result.overall_quality === 'good' ? 'text-emerald-700' : result.overall_quality === 'needs_improvement' ? 'text-amber-700' : 'text-red-700'}`}>
                  {result.overall_quality === 'good' ? '✅ Feedback chất lượng tốt' : result.overall_quality === 'needs_improvement' ? '⚠️ Cần cải thiện' : '❌ Feedback kém chất lượng'}
                </span>
              </div>
              ...
              {result.bias_warning && (
                <div className="p-3 bg-red-50 rounded-lg border border-red-200">
                  <span className="text-[11px] font-medium text-red-700">⚠️ Cảnh báo Bias</span>
